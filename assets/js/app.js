...
function initCordifyApp() {
  (function() {
    ...
    function _fallback_setHistory(arr) {
      try {
        localStorage.setItem(OLD_KEY, JSON.stringify(arr.slice(0, MAX_HISTORY)));
      } catch {}
    }
    ...
    function _fallback_clearHistory() {
      try {
    ...
    function getHistory() {
      if (window.historyStore && typeof window.historyStore.getAll === "function") {
        return window.historyStore.getAll().map(r => ({
          id: r.id,
          ts: r.ts || r.date,
          type: r.type || "",
          input: r.input || "",
          result: r.output || r.result || "",
          date: r.date || r.ts || r.ts
        }));
      }
    ...
    function clearHistory() {
      if (window.historyStore && typeof window.historyStore.clear === "function") {
        try {
    ...
    function removeHistoryAt(indexOrId) {
      if (window.historyStore && typeof window.historyStore.remove === "function" && typeof indexOrId === "string") {
        try {
          return window.historyStore.remove(indexOrId);
        } catch (e) {}
      }
      if (window.historyStore && typeof window.historyStore.getAll === "function" && typeof indexOrId === "number") {
        try {
          ...
          if (rec && rec.id) {
            return window.historyStore.remove(rec.id);
          }
        } catch (e) {}
      }
      if (typeof indexOrId === "number") {
        return _fallback_removeByIndex(indexOrId);
      }
      return false;
    }
    ...
    (() => {
      ...
      function switchTab(tab) {
        ...
        if (mapTabEl) {
          mapTabEl.style.display = "none";
        }
        if (tab === "convert") {
          if (convertTab) {
            convertTab.style.display = "";
          }
      ...
      document.getElementById("map-plot-all")?.addEventListener("click", () => {
        ...
        if (n > 0) {
          ...
          showFeedback("No valid points to plot", "info");
        }
      });
      ...
      dmsInputs.forEach(id => document.getElementById(id)?.addEventListener("keydown", e => {
        if (e.key === "Enter") {
          convertDmsToDd();
        }
      }));
      [ "dd_lat", "dd_lon" ].forEach(id => document.getElementById(id)?.addEventListener("keydown", e => {
        if (e.key === "Enter") {
          convertDdToDms();
        }
      }));
      try {
        ...
        if (pdd) {
          document.getElementById("precision-dd").value = pdd;
        }
        const pdms = localStorage.getItem("cordify_precision_dms");
        if (pdms) {
          document.getElementById("precision-dms").value = pdms;
        }
      } catch {}
      ...
      try {
        ...
        if (last) {
          ...
          [ "dms_lat_string", "dms_lon_string", "dms_lat_deg", "dms_lat_min", "dms_lat_sec", "dms_lon_deg", "dms_lon_min", "dms_lon_sec", "dd_lat", "dd_lon" ].forEach(k => {
            if (v[k] !== void 0 && document.getElementById(k)) {
      ...
      document.getElementById("history-export-json")?.addEventListener("click", () => {
        try {
          ...
          a.href = url;
          a.download = "cordify_history.json";
      ...
      document.getElementById("history-migrate")?.addEventListener("click", () => {
        migrateHistoryToStore();
        alert("Migration started (check console for errors).");
      });
      function persistInputs() {
        ...
        try {
          sessionStorage.setItem("cordify_last_inputs", JSON.stringify(obj));
        } catch {}
      }
      ...
      window._cordify_exportHistory = function exportHistory(type, rowIdx) {
        ...
        let data = arr;
        if (typeof rowIdx === "number" && arr[rowIdx]) {
          data = [ arr[rowIdx] ];
        }
        if (!data.length) {
          return;
        ...
        if (type === "xlsx") {
          if (typeof XLSX === "undefined") {
            ...
            try {
              ...
              XLSX.writeFile(wb, rowIdx !== void 0 ? `cordify_conversion_${rowIdx + 1}.xlsx` : "cordify_history.xlsx");
            } catch {
          ...
          try {
            ...
            const a = document.createElement("a");
            a.href = url;
            a.download = rowIdx !== void 0 ? `cordify_conversion_${rowIdx + 1}.csv` : "cordify_history.csv";
            document.body.appendChild(a);
            a.click();
            setTimeout(() => {
              document.body.removeChild(a);
    ...
    function parseDmsString(dmsStr) {
      ...
      dmsStr = dmsStr.trim().replace(/[′’‘`´]/g, "'").replace(/[″“”]/g, '"').replace(/\u00B0/g, "°").replace(/,/g, ".").toUpperCase().replace(/\s+/g, " ");
      const leadDir = (dmsStr.match(/^[NSWE]/) || [ null ])[0];
      const tailDir = (dmsStr.match(/[NSWE]$/) || [ null ])[0];
      if (leadDir && tailDir && leadDir !== tailDir) {
        return null;
      }
      let dir = tailDir || leadDir || null;
      if (leadDir) {
        dmsStr = dmsStr.slice(1).trim();
      }
      if (tailDir) {
        dmsStr = dmsStr.slice(0, -1).trim();
      }
      ...
      let min = m[2] !== void 0 ? parseFloat(m[2]) : 0;
      let sec = m[3] !== void 0 ? parseFloat(m[3]) : 0;
      if ([ deg, min, sec ].some(Number.isNaN)) {
        return null;
      }
    ...
    function dmsToDd(deg, min, sec, dir) {
      if ([ deg, min, sec ].some(v => Number.isNaN(v))) {
        return null;
      }
      if (min < 0 || min >= 60 || sec < 0 || sec >= 60) {
        return null;
    ...
    function convertDdToDms() {
      ...
      if (Number.isNaN(dd_lat) || !inLatRange(dd_lat)) {
        showErr("err-dd", true);
        return;
      }
      if (Number.isNaN(dd_lon) || !inLonRange(dd_lon)) {
        showErr("err-dd", true);
        return;
      }
    ...
    function populateMapping(headers) {
      ...
      [ ddLat, ddLon, dmsLat, dmsLon ].forEach(sel => {
        sel.innerHTML = "";
        headers.forEach(h => {
    ...
    function runBatch() {
      if (!_batch.rows.length) {
        return;
      }
      const dir = document.getElementById("batch-direction").value;
    ...
    function downloadBatch(type) {
      ...
      if (type === "xlsx") {
        ...
        const ws = XLSX.utils.json_to_sheet(_batch.outRows, {
          header: _batch.outHeaders
        });
        const wb = XLSX.utils.book_new();
        ...
        if (typeof XLSX === "undefined") {
          alert("CSV export requires the XLSX utils (sheetjs).");
          return;
        }
        const ws = XLSX.utils.json_to_sheet(_batch.outRows, {
          header: _batch.outHeaders
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = "cordify_converted.csv";
        document.body.appendChild(a);
        a.click();
        setTimeout(() => {
          document.body.removeChild(a);
    ...
    function parseLatLngFromOutput(s) {
      ...
      if (all && all.length >= 2) {
        const a = parseFloat(all[0]);
        const b = parseFloat(all[1]);
    ...
    function showHistoryItemOnMap(idOrIndex) {
      ...
      if (!rec) {
        ...
        if (typeof idOrIndex === "number") {
          ...
          rec = h.find(r => r.id === idOrIndex) || null;
        }
      }
      if (!rec) {
    ...
    function plotAllHistory() {
      ...
      for (let i = 0; i < batch.length; i += 100) {
        ...
        setTimeout(() => {
          chunk.forEach(({latlng: latlng, label: label}) => {
            try {
              showOnMap(latlng[0], latlng[1], label);
            } catch (e) {}
          });
    ...
    function historyToGeoJSON(items, targetEpsg = "EPSG:4326") {
      ...
      for (const it of items) {
        ...
        if (targetEpsg !== "EPSG:4326" && typeof proj4 !== "undefined") {
          try {
            const p = proj4("EPSG:4326", targetEpsg, [ lon, lat ]);
            features.push({
              type: "Feature",
              geometry: {
                type: "Point",
              ...
              properties: {
                id: it.id || null,
                ts: it.ts || it.date || null,
                input: it.input || "",
                output: it.output || it.result || "",
                type: it.type || ""
              }
            });
            continue;
